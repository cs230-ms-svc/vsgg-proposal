...
class AG(Dataset):
    ...
    def __init__(self, mode, datasize, data_path:str="", filter_nonperson_box_frame=True, filter_small_box=False):
        ...
        for i in video_dict.keys():
            ...
            for j in video_dict[i]:
                if filter_nonperson_box_frame and person_bbox[j]['bbox'].shape[0] == 0:
        ...
        if filter_nonperson_box_frame:
            ...
            print('{} videos are invalid (no person), removed them'.format(self.non_person_video))
            print('{} videos are invalid (only one frame), removed them'.format(self.one_frame_video))
            print('{} frames have no human bbox in GT, removed them!'.format(self.non_gt_human_nums))
        else:
            print('There are {} videos and {} valid frames'.format(len(self.video_list), self.valid_nums))
    ...
    def __getitem__(self, index):
        ...
        for idx, name in enumerate(frame_names):
            ...
            im = im[:, :, ::-1] # rgb -> bgr
            im = prep_im_for_blob(im, DATASET_HEIGHT, DATASET_WIDTH) #cfg.PIXEL_MEANS, target_size, cfg.TRAIN.MAX_SIZE
            processed_ims.append(im)

        blob = im_list_to_blob(processed_ims)
        im_info = np.array([[blob.shape[1], blob.shape[2]]],dtype=np.float32)
